...
try:
    ...
    print('Could not open or write to the run log, Check Error Log')
    ERROR_LOG.write(
...
try:
    ...
    print('Permissions denied, Check Error Log')
...
try:
    ...
    print('File paths not found, Check Error Log')
    ERROR_LOG.write(
...
def main():
    ...
    datetime_start_input = input("Starting date (01-JAN-2025 11:00PM): ")
    date_start = pd.to_datetime(datetime_start_input)
    
    datetime_end_input = input("Ending date (02-JAN-2025 11:00PM): ")
    date_end = pd.to_datetime(datetime_end_input)
    ...
    def filter_by_date(input_file):
        print('Creating daily filter.')
        print(f'Starting datetime: {date_start}.')
        print(f'Ending datetime: {date_end}.')
        filtered_file = input_file
        
        # Try: Filter by date
        try:
            filtered_file = filtered_file[
                ((filtered_file[' DateTime'] >= date_start) &
                (filtered_file[' DateTime'] <= date_end))
            ]
            ...
            print('Filtering Errored, Check Error Log')
            ERROR_LOG.write(textwrap.dedent('''
    ...
    def process_files():
        ...
        try:
            ...
            print('Output Error, Check Error Log')
            ERROR_LOG.write(textwrap.dedent('''
